/*
  # Add archive tables and rollover functionality

  1. New Tables
    - `conference_registrations_archive`
    - `tech_conference_registrations_archive`
    - `hall_of_fame_nominations_archive`
    Each archive table mirrors its source table structure with additional archival metadata

  2. Schema Changes
    - Add `is_active` column to settings tables
    - Add `archived_at` and `archive_id` columns to archive tables
    - Add constraints to ensure only one active settings record

  3. Functions
    - Add functions to handle rollover process
    - Add helper functions for archiving
*/
