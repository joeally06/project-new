...
CREATE POLICY "Enable read access for all users" ON public.membership_applications FOR
SELECT
    USING (TRUE);
...
CREATE POLICY "Allow admin full access" ON public.membership_applications TO authenticated
    USING (EXISTS (
        SELECT
            1
        FROM
            users
        WHERE
            (users.id = uid ()) AND (users.role = 'admin'::text)
    )) WITH CHECK (EXISTS (
        SELECT
            1
        FROM
